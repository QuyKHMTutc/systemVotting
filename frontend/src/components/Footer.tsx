    ...
    <footer className="mt-auto border-t border-white/10 bg-black/20 backdrop-blur-md">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12 lg:py-16">
        <div className="grid grid-cols-1 md:grid-cols-12 gap-10 md:gap-8 lg:gap-16">
          ...
          <div className="md:col-span-4 lg:col-span-5 flex flex-col space-y-5 text-sm text-white/70">
            <h3 className="text-white font-bold text-lg mb-1">{t('footer.contactInfo')}</h3>
            <p className="leading-relaxed">
              <span className="font-semibold text-white/90">{t('footer.addressLabel')}</span> {t('footer.addressValue')}
            </p>
            <p>
              <span className="font-semibold text-white/90">{t('footer.phoneLabel')}</span> {t('footer.phoneValue')}
            </p>
            <p>
              <span className="font-semibold text-white/90">{t('footer.emailLabel')}</span> {t('footer.emailValue')}
            </p>
          </div>

          {/* Column 2: Policies & Terms */}
          <div className="md:col-span-4 lg:col-span-3 flex flex-col space-y-4 text-sm mt-4 md:mt-0">
            <h3 className="text-white font-bold text-lg mb-1">{t('footer.policiesTitle')}</h3>
            <ul className="space-y-4">
              <li>
                <Link to="/privacy" className="text-white/70 hover:text-indigo-400 transition-colors flex items-center gap-3 font-medium">
                  <span className="w-1.5 h-1.5 rounded-full bg-white/30"></span>
                  {t('footer.privacyPolicy')}
                </Link>
              </li>
              <li>
                <Link to="/terms" className="text-white/70 hover:text-indigo-400 transition-colors flex items-center gap-3 font-medium">
                  <span className="w-1.5 h-1.5 rounded-full bg-white/30"></span>
                  {t('footer.termsOfService')}
                </Link>
              </li>
              <li>
                <Link to="/" className="text-white/70 hover:text-indigo-400 transition-colors flex items-center gap-3 font-medium">
                  <span className="w-1.5 h-1.5 rounded-full bg-white/30"></span>
      ...
      <div className="border-t border-white/10 bg-black/40 py-5">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 text-center text-xs sm:text-sm text-white/50 font-medium tracking-wide">
          {t('footer.copyright')}
        </div>
      </div>
    </footer>
